...
\begin{proof}
    ...
    \begin{align*}
    \Psi\colon \{F:K\subseteq F\subseteq E\}&\to\{g\in E[X]:g\text{ is a monic divisor of $f(x,K)$}\},\\
    F&\mapsto f(x,F),
    \end{align*}
    is injective. 
...
\begin{exercise}
If the characteristic of $K$ is different from two and $E/K$ is an extension of degree 2, then it is Galois. 
\end{exercise}
...
\begin{proof}[Proof of Theorem \ref{thm:ArtinGalois}]
    ...
    \]
    Since $f_x\in K[X]$, the extension $E/K$ is normal and separable (as it is a splitting
