...
\begin{example}
    Let $n \in \Z$ be a positive integer and $\mathcal{D}(n)$ be the collection of positive integer divisors $d \in \Z$ of $n$. The relation
    \[
...
and $F,L\in \mathcal{L}(E/K)$. Prove that $\Gal(E/FL)=\Gal(E/F)\cap\Gal(E/L)$ and 
...
\begin{proof}
    ...
    $E_1E_2/E_2$ is algebraic. Since $E_2/K$ is algebraic, $E_1E_2/K$ is algebraic. Similarly, 
...
\begin{exercise}
    ...
    one has $E_j\cap (E_1\cdots E_{j-1}E_{j+1}\cdots E_r)=K$. If $E=E_1E_2\cdots E_r$, then
    \[
...
does not admit extensions of odd degree $>1$. 
...
\begin{proof}
    ...
    is a finite separable extension of even degree. There exists a Galois
    extension 
...
In Chapter \ref{separable} we defined the 
...
\begin{exercise}
    ...
    Moreover, for any $x \in E$, we have that $x^{[E:K]}\in K$. 
\end{exercise}
