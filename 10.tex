...
\begin{proposition}
    ...
    Conversely, 
    if $E/K$ is cyclic of degree $n$, then $E/K$ is a splitting
...
\begin{proof}
    We first prove 1). 
    Let $K_0$ be the prime field of $K$ and $C$ be an algebraic closure of $K$. Note that $K_0\simeq\Z/p$. 
    Let $b\in C$ be a root of $f$ and let $x\in K_0$. 
    Then
    \[
    f(b+x)=(b+x)^p-(b+x)-a
    =(b^p-b-a)+(x^p-x)=0
    \]
    and thus $\{b+x:x\in K_0\}\subset C$ is the set of roots of $f$. Note that 
    ...
    $g=\prod_{x\in S}(X-(b+x))$ in $C[X]$ and hence
    \[
...
\[
    ...
    =\frac{\alpha_{\sigma}(h)}{\alpha_{\sigma}(g)}.
\]
